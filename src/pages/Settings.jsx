...
import { Bell, Calendar, KeyRound, MapPin, MessageCircle, Star } from 'lucide-react'
  ...
  return (
    <button
      ...
      className={`relative h-6 w-11 shrink-0 rounded-full transition ${
        checked ? 'bg-deep-purple' : 'bg-jet-black/15'
      }`}
    >
      <span
    ...
    <div className="min-h-screen bg-soft-white pb-16">
      ...
      <div className="relative isolate overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl space-y-6 px-6 py-8 sm:px-10">
          <div>
            <h1 className="font-display text-2xl font-bold text-jet-black">Settings</h1>
            <p className="mt-1 text-sm text-jet-black/50">Manage how Lendrop notifies you and browses for you.</p>
          </div>

          {loading ? (
            <p className="py-8 text-center text-sm text-jet-black/40">Loading your preferences…</p>
          ) : (
            <>
              {/* ================= NOTIFICATIONS ================= */}
              <section className="rounded-2xl border border-lavender/15 bg-white p-6">
                <div className="mb-4 flex items-center gap-2">
                  <Bell className="h-4 w-4 text-deep-purple" />
                  <h2 className="font-display text-lg font-semibold text-jet-black">Notifications</h2>
                </div>
                <div className="divide-y divide-jet-black/5">
                  {NOTIFICATION_TOGGLES.map(({ key, icon: Icon, label, desc }) => (
                    <div key={key} className="flex items-center justify-between gap-4 py-3.5 first:pt-0 last:pb-0">
                      <div className="flex items-start gap-3">
                        <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-lavender/15">
                ...
                <p className="mt-4 border-t border-jet-black/5 pt-3 text-xs text-jet-black/40">
              ...
              <section className="rounded-2xl border border-lavender/15 bg-white p-6">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="font-display text-lg font-semibold text-jet-black">Browsing</h2>
                </div>
                <label className="mb-1 block text-sm font-medium text-jet-black">Default city on Explore</label>
                <p className="mb-3 text-xs text-jet-black/45">
              ...
              <section className="rounded-2xl border border-lavender/15 bg-white p-6">
                <div className="mb-4 flex items-center gap-2">
                  <KeyRound className="h-4 w-4 text-deep-purple" />
                  <h2 className="font-display text-lg font-semibold text-jet-black">Password</h2>
                </div>
                <form onSubmit={handleChangePassword} className="space-y-3">
                  <div>
                    <label htmlFor="new-password" className="mb-1 block text-sm font-medium text-jet-black">
                      New password
                    </label>
                    <PasswordInput
                      id="new-password"
                  ...
                  <div>
                    <label htmlFor="confirm-password" className="mb-1 block text-sm font-medium text-jet-black">
                  ...
                  <div className="flex items-center justify-end gap-3 pt-1">
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-full border border-jet-black/10 px-5 py-2.5 text-sm font-semibold text-jet-black transition hover:border-lavender hover:text-deep-purple disabled:opacity-50"
                    >

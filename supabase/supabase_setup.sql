...
-- Creates a pending reservation using the server-side item price before the
-- renter is redirected to Wompi. The webhook confirms it after payment.
...
begin
  ...
  values (p_item_id, current_user_id, p_start_date, p_end_date, 'pending', item_price * (p_end_date - p_start_date + 1))
